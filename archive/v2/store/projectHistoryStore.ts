    ...
    loadProjectFromRedis: async (id: string): Promise<Project | null> => {
      try {
        ...
        console.log(
          ...
          responseData.project ? "Project found" : "No project in response",
        );
    ...
    deleteProject: async (id: string) => {
      try {
        ...
        const response = await fetch(
          `/api/projects/${id}?sessionId=${encodeURIComponent(sessionId)}`,
          {
            method: "DELETE",
          },
        );
      ...
      try {
        ...
        if (!forceRefresh && lastLoadedAt) {
          ...
          if (cacheAge < CACHE_STALE_TIME_MS) {
            console.log(
              `📋 Using cached projects (${Math.round(cacheAge / 1000)}s old)`,
            );
        ...
        const response = await fetch(
          `/api/projects?sessionId=${encodeURIComponent(sessionId)}`,
        );
      ...
      try {
        ...
        if (response.ok) {
          ...
          for (const project of projects || []) {
            await fetch(
              `/api/projects/${project.id}?sessionId=${encodeURIComponent(sessionId)}`,
              {
                method: "DELETE",
              },
            );
