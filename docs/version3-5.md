...
- **Persistent mute / solo state.** Mute/solo shipped in stage 8d but state is ephemeral (`mutedTrackIds` / `soloedTrackIds` Sets in `mixerStore`, not written to Redis). If session-survival is wanted, lift onto the mixer draft as a `transientControls` field.
