...
- Single orchestrator: OpenAI GPT-5.5 with Chain-of-Thought continuity
...
V3 uses **GPT-5.5 as the sole orchestrator**. We trust its Chain-of-Thought reasoning via `previous_response_id` continuity instead of building manual guardrails.
...
└─────────────────────────────────────────┘
    ↓
GPT-5.5 decides when complete (CoT continuity)
    ↓
Redis (V3_REDIS_URL)
```
...
**Single orchestrator:** OpenAI GPT-5.5 via Responses API
...
- ✅ Single orchestrator: OpenAI GPT-5.5 only (deleted Qwen/Kimi adapters)
...
- GPT-5.5 remains sole conductor
