...
- Single orchestrator: OpenAI GPT-5.4 with Chain-of-Thought continuity
...
V3 uses **GPT-5.4 as the sole orchestrator**. We trust its Chain-of-Thought reasoning via `previous_response_id` continuity instead of building manual guardrails.
...
└─────────────────────────────────────────┘
    ↓
GPT-5.4 decides when complete (CoT continuity)
    ↓
Redis (V3_REDIS_URL)
```
...
**Single orchestrator:** OpenAI GPT-5.4 via Responses API
...
- ✅ Single orchestrator: OpenAI GPT-5.4 only (deleted Qwen/Kimi adapters)
...
- GPT-5.4 remains sole conductor
