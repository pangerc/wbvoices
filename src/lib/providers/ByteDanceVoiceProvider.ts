...
export class ByteDanceVoiceProvider extends BaseAudioProvider {
  ...
  async makeRequest(
    ...
    console.log(
      `  Voice Instructions: ${voiceInstructions ? (voiceInstructions as string).substring(0, 50) + "..." : "none"}`,
    );
    ...
    for (const line of lines) {
      ...
      try {
        ...
        console.warn(
          `ByteDance: Failed to parse response line: ${line.substring(0, 100)}`,
        );
        continue;
      }
      ...
      if (chunk.code !== 0) {
        return {
          success: false,
          error: `ByteDance API error (${chunk.code}): ${chunk.message || "Unknown error"}`,
        };
      }
    ...
    console.log(
      `ByteDance TTS 2.0: Got ${audioChunks.length} chunks, ${audioBuffer.byteLength} bytes total`,
    );
