...
export abstract class BaseAudioProvider {
  ...
  async handleRequest(req: NextRequest): Promise<NextResponse> {
    try {
      ...
      if (validation.data?.isInternalRequest) {
        console.log(
          `${this.providerName}: Processing ready track for blob upload...`,
        );
      ...
      if (!this.validateCredentials()) {
        return this.createErrorResponse(
          `${this.providerName} API credentials are missing`,
          500,
        );
      }
      ...
      return this.createErrorResponse(
        error instanceof Error
          ? error.message
          : `Unknown error in ${this.providerName} provider`,
        500,
      );
    }
  }
  ...
  protected async handleApiError(
    response: Response,
  ): Promise<{ message: string; details?: ErrorDetails }> {
    ...
    try {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        console.error(
          `${this.providerName} detailed error response:`,
        ...
        console.error(
          `${this.providerName} non-JSON error response:`,
      ...
      console.error(
        `Error parsing ${this.providerName} error response:`,
        parseError,
      );
    }
