...
export class MubertProvider extends BaseAudioProvider {
  ...
  async authenticate(): Promise<AuthCredentials> {
    ...
    const response = await this.makeFetch(
      `${this.MUBERT_BASE_URL}/service/customers`,
      {
        method: "POST",
        headers: {
    ...
    if (!customerId || !accessToken) {
      throw new Error(
        "Failed to extract customer credentials from registration response",
      );
    }
  ...
  async makeRequest(
    ...
    console.log(
      `🔍 Mubert: Checking cache for prompt: "${prompt}" (${roundedDuration}s)`,
    );
    const cached = await checkMusicCache(
      prompt as string,
      "mubert",
      roundedDuration,
    );
    ...
    if (cached) {
      console.log(
        `💰 Mubert: Cache HIT! Saved $$ by using cached music: ${cached.url}`,
      );
      return {
        success: true,
        data: {
    ...
    console.log(
      `💸 Mubert: Cache MISS - Generating NEW music ($$$ spent): "${prompt}" (${roundedDuration}s)`,
    );

    const response = await this.makeFetch(
      `${this.MUBERT_BASE_URL}/public/tracks`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async pollStatus(
    ...
    const response = await this.makeFetch(
      `${this.MUBERT_BASE_URL}/public/tracks/${taskId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  public async processSuccessfulResponse(
    ...
    if (generation_url && typeof generation_url === "string") {
      try {
        console.log(
          "💸 Mubert: NEW music generated, uploading to Vercel Blob with cache key...",
        );
        ...
        console.log(
          `Mubert music uploaded to blob (cached=${blobResult.cached}): ${blobResult.url}`,
        );
