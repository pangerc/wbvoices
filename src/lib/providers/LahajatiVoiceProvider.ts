...
export class LahajatiVoiceProvider extends BaseAudioProvider {
  ...
  validateParams(body: Record<string, unknown>): ValidationResult {
    ...
    return {
      ...
      data: {
        ...
        voiceInstructions:
          typeof voiceInstructions === "string" ? voiceInstructions : undefined,
      },
    };
  }
   ...
   * 3. Default to MSA (1)
   */
  private async resolveDialectId(
    dialectId?: number,
    accent?: string,
  ): Promise<number> {
  ...
  async makeRequest(
    ...
    console.log(
      `  Dialect ID: ${resolvedDialectId}${dialectId ? " (explicit)" : " (resolved from accent)"}`,
    );
    console.log(
      `  Performance ID: ${resolvedPerformanceId}${performanceId ? " (explicit)" : ` (from style: ${style || "neutral"})`}`,
    );
    console.log(
      `  Voice Instructions: ${voiceInstructions ? "provided" : "none"}`,
    );
    ...
    if (voiceInstructions && typeof voiceInstructions === "string") {
      ...
      console.log(
        `  📝 Using input_mode "1" (custom prompt): ${customPrompt.substring(0, 100)}...`,
      );
    } else {
      ...
      console.log(
        `  📋 Using input_mode "0" (structured): performance_id=${resolvedPerformanceId}, dialect_id=${resolvedDialectId}`,
      );
    }
    ...
    console.log(
      `✅ Lahajati TTS success: received ${audioArrayBuffer.byteLength} bytes`,
    );
