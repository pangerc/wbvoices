...
export async function generateCreativeCopy(
  ...
  console.log("Generating creative copy with DeepSeek API...");
...
DO NOT include any explanations, markdown formatting, or additional text outside the XML structure.`;
  ...
  try {
    ...
    // Direct fix for the specific format we're seeing
    if (
      responseContent.includes("<voice id=\\") ||
      responseContent.includes('<voice id="\\')
    ) {
      console.log("Detected escaped quotes in voice ID, applying direct fix");
      // Replace all instances of escaped quotes in voice ID attributes
  ...
  <music>
    <prompt>Background music that matches the tone of the advertisement, approximately 60 seconds in duration.</prompt>
  </music>
</creative>`;
