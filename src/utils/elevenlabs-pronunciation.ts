...
export async function createDictionary(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to create pronunciation dictionary: ${response.status} ${errorText}`,
    );
  }
...
export async function getDictionary(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to get pronunciation dictionary: ${response.status} ${errorText}`,
    );
  }
...
export async function listDictionaries(): Promise<PronunciationDictionary[]> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to list pronunciation dictionaries: ${response.status} ${errorText}`,
    );
  }

  const data = (await response.json()) as DictionaryListResponse;
  console.log(
    `✅ Found ${data.pronunciation_dictionaries.length} dictionaries`,
  );
...
export async function removeRules(
  ...
  console.log("📖 Removing rules from pronunciation dictionary:", {
    dictionaryId,
    count: ruleStrings.length,
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to remove pronunciation rules: ${response.status} ${errorText}`,
    );
  }
  ...
  console.log("✅ Rules removed:", {
    ...
    rules_remaining: result.version_rules_num,
  });
...
export async function addRules(
  ...
  console.log("📖 Adding rules to pronunciation dictionary:", {
    dictionaryId,
    count: rules.length,
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to add pronunciation rules: ${response.status} ${errorText}`,
    );
  }
  ...
  console.log("✅ Rules added:", {
    version_id: result.version_id,
    total_rules: result.version_rules_num,
  });
 ...
 */
export function validateRules(rules: PronunciationRule[]): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  rules.forEach((rule, index) => {
    if (!rule.string_to_replace || typeof rule.string_to_replace !== "string") {
      errors.push(
        `Rule ${index}: string_to_replace is required and must be a string`,
      );
    }
    ...
    if (
      rule.type === "alias" &&
      (!rule.alias || typeof rule.alias !== "string")
    ) {
    ...
    if (
      rule.type === "phoneme" &&
      (!rule.phoneme || typeof rule.phoneme !== "string")
    ) {
    ...
    if (
      rule.type === "phoneme" &&
      rule.alphabet &&
      !["ipa", "cmu", "x-sampa"].includes(rule.alphabet)
    ) {
