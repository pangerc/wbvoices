...
export async function GET() {
  try {
    ...
    const countsTower =
      ((await redis.get("counts_tower")) as Record<
        string,
        Record<
          string,
          Record<
    ...
    for (const language of languages) {
      ...
      const languageTotal =
        providerTotals.elevenlabs +
        providerTotals.lovo +
        providerTotals.openai +
        providerTotals.qwen +
        providerTotals.bytedance +
        providerTotals.lahajati;
      ...
      for (const region of regions) {
        Object.keys(languageData[region]).forEach((accent) =>
          accents.add(accent),
        );
      }
    ...
    // Get sample voices
    const dataTower =
      ((await redis.get("voice_data_tower")) as Record<
        string,
        {
          displayName?: string;
          language?: string;
      ...
      >) || {};
    const sampleVoices = Object.values(dataTower)
      .slice(0, 5)
      .map((voice) => ({
