...
export async function GET(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to get dictionary";
...
export async function DELETE(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const rulesData = await redis.get<{
    ...
    if (rulesData && rulesData.rules && rulesData.rules.length > 0) {
      ...
      const ruleStrings = rulesData.rules
        .map((r) => r.stringToReplace)
        .filter(Boolean);

      if (ruleStrings.length > 0) {
        console.log(
          `🗑️ Removing ${ruleStrings.length} rules from ElevenLabs dictionary...`,
        );
        try {
          ...
          console.error(
            "⚠️ Failed to remove rules from ElevenLabs:",
            removeError,
          );
...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.log("📖 Updating pronunciation dictionary:", {
      id,
    ...
    if (rulesData && rulesData.rules && rulesData.rules.length > 0) {
      const oldRuleStrings = rulesData.rules
        .map((r) => r.stringToReplace)
        .filter(Boolean);
    ...
    if (rules.length > 0) {
      ...
      const apiRules: ApiPronunciationRule[] = rules.map(
        (rule: {
          ...
          string_to_replace: (rule.stringToReplace ||
            rule.string_to_replace) as string,
