...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `🔄 Restoring ${voiceIds.length} voices for language=${language}`,
    );
    ...
    for (const provider of providers) {
      try {
        ...
        const mappedVoices: Voice[] = providerVoices.map(
        ...
        console.error(
          `Failed to load ${provider} voices for restoration:`,
          error,
        );
    ...
    console.log(
      `✅ Restored ${restoredVoices.length}/${voiceIds.length} voices`,
    );
